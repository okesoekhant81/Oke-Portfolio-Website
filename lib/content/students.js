...
export async function getStudents({ includeDeleted = false } = {}) {
  ...
  return includeDeleted ? students : students.filter((s) => !s.deletedAt)
}
...
export async function deleteStudent(id) {
  await mutateJson(PATH, (existing) =>
    (existing || []).map((s) => (s.id === id ? { ...s, deletedAt: new Date().toISOString() } : s))
  )
}

export async function restoreStudent(id) {
  await mutateJson(PATH, (existing) => (existing || []).map((s) => (s.id === id ? { ...s, deletedAt: null } : s)))
}
