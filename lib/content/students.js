...
export async function addStudent(fields) {
  ...
  const sql = db()
  try {
    // Pass the plain object, not JSON.stringify(record) — postgres.js
    // infers this parameter's type as jsonb from the `::jsonb` cast and
    // serializes it exactly once. Passing an already-stringified value
    // here double-encodes it (the column ends up holding a jsonb *string*
    // containing escaped JSON text, not a jsonb *object*), which is what
    // caused every field except id to disappear the first time this
    // shipped.
    await sql`INSERT INTO students (id, data) VALUES (${record.id}, ${record}::jsonb)`
  } catch (err) {
    // app/actions/students.js shows a thrown error's .message straight to
    // the admin submitting the form — safe for Blob, not for a raw
    // Postgres error.
    console.error('addStudent failed:', err)
    throw new Error('Could not add. Please try again.')
  }
  return record
}
...
// Cheap "did anything change" check for AutoRefresh's poll — same
// reasoning as getInquiriesFingerprint in lib/content/inquiries.js.
export async function getStudentsFingerprint() {
  if (!isDbConfigured) return null
  try {
    const sql = db()
    const rows = await sql`SELECT count(*)::int AS count, max(data->>'createdAt') AS latest FROM students`
    return { count: rows[0]?.count ?? 0, latest: rows[0]?.latest ?? null }
  } catch (err) {
    console.error('getStudentsFingerprint failed:', err)
    return null
  }
}

// cache()'d per request — /verify/[id] calls this from both
// generateMetadata (for the Open Graph preview when someone shares their
// certificate) and the page body itself; without this, that's two full
// getStudents() table reads for the same id on every single page view.
export const getStudent = cache(async function getStudent(id, { includeDeleted = false } = {}) {
