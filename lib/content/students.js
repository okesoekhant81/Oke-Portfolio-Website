...
export async function getStudents({ includeDeleted = false } = {}) {
  ...
  try {
    ...
    const rows = includeDeleted
      ? await sql`SELECT data FROM students ORDER BY data->>'createdAt' DESC`
      : await sql`SELECT data FROM students WHERE data->>'deletedAt' IS NULL ORDER BY data->>'createdAt' DESC`
...
export async function addStudent(fields) {
  ...
  const sql = db()
  await sql`INSERT INTO students (id, data) VALUES (${record.id}, ${JSON.stringify(record)}::jsonb)`
  return record
}
...
export async function updateStudent(id, patch) {
  const sql = db()
  await sql`UPDATE students SET data = data || ${JSON.stringify(patch)}::jsonb WHERE id = ${id}`
}
...
export async function setAttendance(studentId, date, present, note) {
  ...
  await sql.begin(async (tx) => {
    ...
    await tx`UPDATE students SET data = jsonb_set(data, '{attendance}', ${JSON.stringify(attendance)}::jsonb) WHERE id = ${studentId}`
  })
}
