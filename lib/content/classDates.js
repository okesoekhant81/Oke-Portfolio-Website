...
export async function addClassDate({ date, label, time, meetingLink, defaultFee, capacity }) {
  const record = {
    ...
    time: time || '',
    // Per class, not a single site-wide link (like sessionPlatform/
    // sessionLanguage in workshop content are) — a Google Meet link is
    // normally unique per session, not reused across every class.
