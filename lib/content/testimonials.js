...
export async function getTestimonials({ includeDeleted = false } = {}) {
  if (!isBlobConfigured) return []
  const testimonials = (await readJson(PATH)) || []
  return includeDeleted ? testimonials : testimonials.filter((t) => !t.deletedAt)
}
...
// gone the instant someone clicks Delete. permanentlyDeleteTestimonial is
// the old hard-delete behavior, only reachable from Trash itself.
export async function deleteTestimonial(id) {
  await mutateJson(PATH, (existing) =>
    (existing || []).map((t) => (t.id === id ? { ...t, deletedAt: new Date().toISOString() } : t))
  )
}

export async function restoreTestimonial(id) {
  await mutateJson(PATH, (existing) => (existing || []).map((t) => (t.id === id ? { ...t, deletedAt: null } : t)))
}

export async function permanentlyDeleteTestimonial(id) {
