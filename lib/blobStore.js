...
// app/actions/backup.js) and the secured-path migration (see
// app/actions/storageMigration.js) — the exact text of a blob, with no JSON
// parse/reserialize round-trip and no business-logic filtering (soft-
// deleted records, drafts, etc. all included), so a snapshot or a copy is a
// true byte-for-byte match of what's actually stored.
...
// Only for the secured-path migration — writes exact text to an explicit
// pathname, bypassing JSON.stringify (the caller already has raw text from
// readJsonRaw, so re-parsing and re-serializing it would just be a chance
// to subtly change it).
export async function writeJsonRaw(pathname, text) {
  if (!isBlobConfigured) {
    throw new Error('Storage is not connected yet — connect a Vercel Blob store first.')
  }
