...
// React's cache() memoizes per pathname for the lifetime of a single
// request/render — it doesn't persist across requests, so this can't ever
// serve one visitor's response stale data from another's. What it does fix:
// generateMetadata() and the page component both reading the same content
// (about.json, workshop.json, ...), or a page reading classDates.json
// twice for two different reasons, used to mean two Blob round-trips for
// data that couldn't have changed between them — now it's one.
// mutateJson below deliberately calls readJsonWithEtag directly, not this,
// since its compare-and-swap needs a genuinely fresh read/etag every
// attempt, not a cached one.
export const readJson = cache(async function readJson(pathname) {
...
// Uploaded images (see lib/uploadImage.js) are stored plain — not under
// securedPath()'s BLOB_PATH_SECRET prefix, unlike content JSON — since
// they're meant to be publicly fetchable directly by the browser, and
// already get a random suffix at upload time so their URLs aren't
// guessable. del() accepts the full public URL directly, no pathname
// extraction needed.
export async function deleteBlobByUrl(url) {
  try {
    await del(url)
  } catch (err) {
    console.error('deleteBlobByUrl failed:', url, err)
  }
}
...
// Same as listBlobs, but follows the `cursor`/`hasMore` pagination through
// to the end instead of stopping at the first 100 — for anything that needs
// a genuinely complete listing (e.g. the orphaned-image report in
// app/actions/imageReport.js), not just what fits on one admin picker page.
