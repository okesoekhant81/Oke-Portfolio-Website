...
export async function sendRegistrationConfirmation(student, classInfo, workshop, locale = 'en') {
  ...
  // classInfo.meetingLink is already restricted to a real http(s) URL by
  // cleanUrl in app/actions/classDates.js — safe to drop straight into an
  // href, same trust level as the admin-entered fields above it.
  const meetingLinkRow = classInfo.meetingLink
    ? detailRow(
        isMy ? 'အစည်းအဝေး Link' : 'Meeting Link',
        `<a href="${classInfo.meetingLink}" style="color:#e83606">${classInfo.meetingLink}</a>`
      )
    : ''
...
// Sent once per student, the moment their class's status is changed to
// 'completed' (see updateClassDateStatusAction in app/actions/classDates.js)
// — only ever called for students who were actually paid, since an unpaid
// registration was never issued a real certificate in the first place (see
// the `verified` check in app/verify/[id]/page.jsx, which requires both).
export async function sendCertificateReady(student, classInfo, workshop, locale = 'en') {
  if (!student.email) return

  const isMy = locale === 'my'
  const verifyUrl = `${SITE_URL}/verify/${student.id}`
  const courseName = classInfo.label || `${workshop.heroTitle} ${workshop.heroSubtitle}`.trim()

  const bodyEn = `
    <p style="margin:0 0 12px">Hello ${student.name},</p>
    <p style="margin:0 0 12px">${workshop.certificateReadyBody}</p>
    <p style="margin:0 0 12px;color:#737373;font-size:13px">${courseName}</p>
    <p style="margin:0 0 20px"><a href="${verifyUrl}" style="color:#e83606;font-weight:700">View your certificate</a></p>
    <p style="margin:0">Best regards,<br/>${SITE_NAME}<br/>${workshop.paymentConfirmedSignature}</p>
  `
  ...
  const bodyMy = `
    ...
    <p style="margin:0 0 12px">${workshop.certificateReadyBody}</p>
    <p style="margin:0 0 12px;color:#737373;font-size:13px">${courseName}</p>
    <p style="margin:0 0 20px"><a href="${verifyUrl}" style="color:#e83606;font-weight:700">Certificate ကြည့်ရန်</a></p>
    <p style="margin:0">လေးစားစွာဖြင့်၊<br/>${SITE_NAME}<br/>${workshop.paymentConfirmedSignature}</p>
  `

  await sendEmail({
    to: student.email,
    subject: workshop.certificateReadySubject,
    html: wrap(isMy ? bodyMy : bodyEn),
  })
}
    ...
    html: wrap(`
      ...
      ${
        classInfo.meetingLink
          ? `<p style="margin:0"><a href="${classInfo.meetingLink}" style="color:#e83606">${isMy ? 'အစည်းအဝေးထဲ ဝင်ရန်' : 'Join the meeting'}</a></p>`
          : ''
      }
    `),
  })
