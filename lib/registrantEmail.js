...
export async function sendRegistrationConfirmation(student, classInfo, workshop, locale = 'en') {
  ...
  const detailsTable = `<table style="margin:16px 0;border-collapse:collapse;font-size:14px">
    ${detailRow(isMy ? 'မှတ်ပုံတင်အမှတ်' : 'Registration ID', student.id?.toUpperCase())}
  ...
  const bodyEn = `
    ...
    ${detailsTable}
    <p style="margin:0 0 12px">This workshop is designed to help you move from simply doing marketing to thinking more strategically about business growth, starting from business fundamentals and building through customer understanding, branding, marketing, and growth.</p>
    ...
    <p style="margin:0">Best regards,<br/>Oke Soe Khant<br/>Strategist / Workshop Instructor</p>
  `
  ...
  const bodyMy = `
    ...
    ${detailsTable}
    <p style="margin:0 0 12px">ဒီ workshop ဟာ marketing ကို ရိုးရိုးလုပ်နေရာကနေ business growth အတွက် ဗျူဟာမြောက် တွေးခေါ်တတ်အောင် ကူညီပေးဖို့ ရည်ရွယ်ပါတယ် — business fundamentals ကနေစပြီး customer understanding, branding, marketing နဲ့ growth ထိ ဆက်စပ်တည်ဆောက်ပေးထားပါတယ်။</p>
    ...
    <p style="margin:0">လေးစားစွာဖြင့်၊<br/>Oke Soe Khant<br/>Strategist / Workshop Instructor</p>
  `

  await sendEmail({
    to: student.email,
    subject: isMy ? `${SITE_NAME} — သင့်ရဲ့ Workshop စာရင်းသွင်းမှု အတည်ပြုပြီးပါပြီ` : 'Your Workshop Registration Is Confirmed',
